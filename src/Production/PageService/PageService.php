...
class PageService implements Production
{
    ...
    {
        try {
            $store = $attributes ? DBPageService::create([
     ...
     * 
     */
    public function createNewFirstPageService(array $service): bool
    {
        $result = false;
        // page Service ...
        $hasIndex = DBPageService::where('service_id', '=', $service['id'])->first();
        if(! $hasIndex || empty($hasIndex)){
            $ps = $this->createNewPageService([
                'serviceId' => $service['id'],
                'routeName' => ClientWebRoute::INDEX->value,
                'name' => 'Home',
                'title' => $service['alias'],
